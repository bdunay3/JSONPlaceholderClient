...
public protocol FileLoadable: Decodable {}

public func loadObject<T: FileLoadable>(from fileName: String, `extension`: String = "json") -> T? {
...
public func loadObject<T: FileLoadable>(from fileName: String, `extension`: String = "json", inBundle bundle: Bundle) -> T? {
...
public func loadList<T: FileLoadable>(from fileName: String, `extension`: String = "json") -> [T]? {
    guard let frameworkBundle = Bundle(identifier: frameworkIdentifier) else {
        assertionFailure("\(frameworkIdentifier) is not a valid Bundle Identifier in this project!")
        return nil
    }
    
    return loadList(from: fileName, extension: `extension`, inBundle: frameworkBundle)
}

public func loadList<T: FileLoadable>(from fileName: String, `extension`: String = "json", inBundle bundle: Bundle) -> [T]? {
    let fileManager = FileManager.default
